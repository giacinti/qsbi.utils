...
##############################################################################
def add_if(db: Session, orm_class: models.Base, attr, **kwargs) -> Optional[models.Base]:  # type: ignore
    """ add object in db if not already in, filter with 'attr' """
    obj: Optional[models.Base] = db.query(orm_class).filter(getattr(orm_class,attr) == kwargs[attr]).first()  # type: ignore
    if not obj:
